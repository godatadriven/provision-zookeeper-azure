...
sed -i "s|\(<property name=\"Initial User Identity 1\">\).*||g" $NIFI_HOME_DIR/conf/authorizers.xml

for (( c=0; c<$2; c++ ))
do
    sed -i '/<\/userGroupProvider>/i \
    <property name="Initial User Identity '$c'">CN=nifi'$c', OU=NIFI<\/property>
    ' $NIFI_HOME_DIR/conf/authorizers.xml
done

sed -i '/<\/userGroupProvider>/i \
    <property name="Initial User Identity 3">L=Utrecht, C=NL, CN=NiFi Admin<\/property>
    ' $NIFI_HOME_DIR/conf/authorizers.xml
